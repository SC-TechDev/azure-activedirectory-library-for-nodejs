...
TokenRequest.prototype._createCacheQuery = function() {
  ...
  if (this._userId) {
    ...
    // In order to combine the use of on-behalf-of tokens with
    // the use of service principal tokens, we need to make sure that
    // we don't retrieve based on just clientId as the on-behalf-of-token
    // tokens will share the same client Id as the service principal token
