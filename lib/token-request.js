...
TokenRequest.prototype.getTokenWithClientCredentials = function(clientSecret, callback) {
  ...
  //The problem is that adal wants to cache the token based off of
  //its client ID, and expects it to be unique in the cache
  //however, on-behalf-of tokens share the same client ID
  //so we need another key to differentiate them
  this._userId = clientSecret;
